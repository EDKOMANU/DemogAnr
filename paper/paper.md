...
Because the package is intended for the production of official statistics rather than for exploratory modelling, each implemented method is verified against an external standard rather than against its own output. Where a method has a published worked example, the test suite reproduces it. The Brass P/F ratio is checked on the Bangladesh 1974 schedule of @un1983 (Chapter II), indirect child mortality on Panama 1976 (Chapter III), and maternal orphanhood on Bolivia 1975 (Chapter IV); the crude death rate, the cohort-component projection, the singulate mean age at marriage and direct age standardization are checked on the worked examples of @preston2001 (Boxes 1.2, 6.1, 4.4 and 2.1 respectively); and the United Nations age-sex accuracy index on the Ghana 1960 census example of @kpedekpo1982. The orphanhood implementation reproduces its published example exactly: all seven weighting factors to the four decimal places published, and the seven survivorship ratios and six time-reference periods to the precision at which *Manual X* reports them [@brasshill1973].

Methods for which the literature supplies a construction rather than a numerical example are instead verified against the analytical identities they must satisfy: the Kitagawa and Arriaga decompositions, for instance, are required to sum exactly to the difference in crude rates and in life expectancy that each is decomposing.

Where neither a published example nor a closed identity is available, verification uses a synthetic population whose true value is known by construction. The death distribution methods are tested against a stable population obtained by integrating a Siler mortality schedule on a fine age grid, from which a known fraction of deaths is withheld to simulate incomplete registration; the generalized growth balance, synthetic extinct generations, and combined procedures each recover the withheld fraction to within one per cent for completeness between 0.3 and 1.0. The relational logit implementation is verified by the requirement that the identity transformation reproduce its own standard, which it does to the precision at which the standard is tabulated.

Every one of the package's 37 exported functions is covered by the test suite, which comprises more than 500 assertions and is run on each commit against macOS, Windows and Linux, on the current, development, and previous release versions of R as well as the oldest version the package declares support for.
